...
def Calc_Profit(profit: list, weight: list, max_Weight: int) -> Union[str, int]:
    ...
    if len(profit) != len(weight):
        raise IndexError("<< The length of both the arrays must be same! Try again.. >>")

    if max_Weight <= 0:
        raise ValueError("<< Gotcha! Max_Weight is a positive quantity greater than zero! >>")

    for i in range(len(profit)):
        if profit[i] < 0:
            raise ValueError("<< Ono! Profit means positive value. Better luck next time! >>")

        if weight[i] < 0:
            raise ValueError("<< Oops! Could not accept a negative value for weight. Try Again.. >>")
