...
int main() {
  ...
  while (u > l) {
    ...
    if (max <
        arr[index][mid - 1]) // if it is smaller than left element,search left
    {
      u = mid - 1;
    }
    if (max < arr[index][mid + 1]) // if right element is greater,search right
    {
      ...
      cout << arr[index][mid]; // else we found the element because since, it is
                               // greatest element,it is greater then
                               // up,down,left and right element
      break;
    }
  }
}
