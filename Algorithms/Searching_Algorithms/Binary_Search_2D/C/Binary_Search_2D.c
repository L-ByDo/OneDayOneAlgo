 ...
 * ----------------------
 * checks ITERATIVELY for for the target value in array and prints the index if
 * found + the number of comparisons made.
 *
...
int main(void) {
    ...
    for (i = 0; i < rows; i++) {
        ...
        // If not first Row
            ...
            for (j = 0; j < columns; j++) {
                ...
                // value of this element is more than that of last element of previous
                // row
                if (j == 0) {
    ...
    printf(
        "Your randomly sorted array is ready!Enter a number below to search. \n");
    int target;
    scanf("%d", &target);
